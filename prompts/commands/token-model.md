...
Read `docs/ARCHITECTURE_OVERVIEW.md` to understand components, data flows, and MCP tool surfaces. Also read `docs/ARCHITECTURE_DETAIL.md` when analyzing security, network, or infrastructure token costs. Map each component interaction to a token-consuming operation (LLM call, tool call, file read, test poll).
...
2. **Architecture Review** (Rule 2): Read `docs/ARCHITECTURE_OVERVIEW.md` to understand components, data flows, and MCP tool surfaces. Also read `docs/ARCHITECTURE_DETAIL.md` when analyzing security, network, or infrastructure token costs. Map the request lifecycle to token-consuming operations. Identify which components generate LLM round-trips.
