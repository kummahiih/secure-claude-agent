...
(cd mcp && python -m pytest files_mcp_test.py git_mcp_test.py tester_mcp_test.py -v --tb=short 2>&1 | grep -E '(PASSED|FAILED|ERROR|test_|===)')

echo "[unit] Running Python claude tests..."
(cd claude && python -m pytest claude_tests.py test_isolation.py log_mcp_test.py test_server.py -v --tb=short 2>&1 | grep -E '(PASSED|FAILED|ERROR|test_|===)')